...
public class GuitarSoundFont : ISoundFont
{
    public string Name => "吉他";

    public ISampleProvider CreateNoteProvider(double frequency, double volume, double durationMs, int noteNumber)
    {
        return new GuitarNoteProvider(frequency, volume, durationMs, noteNumber);
    }

    private class GuitarNoteProvider : ISampleProvider
    {
        ...
        private const double AttackMs = 10;
        private const double DecayMs = 300;
        private const double SustainLevel = 0.4;
        private const double ReleaseMs = 800;
        ...
        public GuitarNoteProvider(double frequency, double volume, double durationMs, int noteNumber)
        {
        ...
        private void InitializePartials()
        {
            ...
            // 吉他的非谐性系数
            double inharmonicityB = 0.0001 * Math.Pow(_frequency / 100, 1.2);
            ...
            for (int i = 0; i < numPartials; i++)
            {
                ...
                // 振幅包络：吉他的泛音结构
                double amplitudeDecay = Math.Pow(n, -1.2);
                
                // 根据音高调整音色
                if (_noteRatio < 0.3) // 低音区
                {
                    amplitudeDecay *= (1.0 + 0.3 / n); // 增强低频泛音
                }
                ...
                // 衰减率：高频泛音衰减更快
                _partialDecays[i] = 0.3 + n * 0.3 + n * n * 0.03;
        ...
        public int Read(float[] buffer, int offset, int sampleCount)
        {
            ...
            for (int i = 0; i < sampleCount && !_stopped; i += channels)
            {
                ...
                double sample = GenerateGuitarSample(timeMs);
        ...
        private double GenerateGuitarSample(double timeMs)
        {
            double sample = 0;
            ...
            if (timeMs < 15)
            {
                ...
                // 中频噪声更多
                double noiseAmount = noiseDecay * 0.15 * (1.0 + 0.3 * _noteRatio);
                pluckNoise = (_random.NextDouble() * 2 - 1) * noiseAmount;
            }

            // 琴弦共振效果
            double resonance = 0;
            if (timeMs > 50 && timeMs < 500)
            {
                double resonanceDecay = Math.Exp(-(timeMs - 50) / 200.0);
                resonance = Math.Sin(_frequency * timeSec * 2 * Math.PI + _random.NextDouble() * 0.1) * resonanceDecay * 0.05;
            }
        ...
        private double CalculateEnvelope(double timeMs)
        {
            ...
            if (timeMs >= releaseStart)
            {
                ...
                return SustainLevel * Math.Exp(-3 * releaseProgress);
            }

            if (timeMs < AttackMs)
            {
                double attackProgress = timeMs / AttackMs;
                return Math.Pow(attackProgress, 0.5);
            }
